...
export default function convertTxtToSong(
  ...
  const additionalData = {
    ...
    manualVolume: getPropertyValueFromTxt(text, ['ALLKARAOKE_OLDVOLUME'], 'number'),
  ...
  const rawVideo = getPropertyValueFromTxt(text, ['VIDEO']) ?? '';
  const isLocalVideo = rawVideo.startsWith('/songs/') || rawVideo.endsWith('.mp4') || rawVideo.endsWith('.webm');
  const video = isLocalVideo
    ? rawVideo
